...
| 用户名 | 你的坚果云邮箱 |
| 密码 | 应用密码（坚果云 → 账户信息 → 安全选项 → 添加应用密码） |

**Infini-Cloud**（日本服务商，免费 20GB）
| 字段 | 值 |
|------|-----|
| 服务器地址 | 你的个人 WebDAV 地址（`https://xxxx.teracloud.jp/dav/`，在 [My Page](https://infini-cloud.net/en/modules/mypage/) 查看） |
| 用户名 | 你的 User ID（在 My Page 查看） |
| 密码 | Apps Password（My Page → Turn on Apps Connection → 生成，不是登录密码） |
